...
const test = {
  ...
  manipulateDom() {
    ...
    const txt = '<div id="copy-link-message" class="notice copy-link-hidden"><p /><a class="notice-close sprite2" title="Stäng"></a></div>';
    $('.cro #bottom-bar').prepend(txt);
    $('.cro #bottom-bar a.notice-close').on('click', function(e) {
      $('.cro #copy-link-message').addClass('copy-link-hidden');
    })
